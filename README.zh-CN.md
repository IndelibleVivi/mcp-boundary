...
[English README](README.md) · [官网源码](site/) · [当前状态](docs/current-state.md) · [许可边界](LICENSING.md)
...
安装后重启 Codex。可分发 package 已提交在 [`plugins/mcp-boundary/`](plugins/mcp-boundary/)，root manifest 是 `plugin.json`。
...
最后一条只在 ignored `dist/` 里生成 deterministic ZIP 和 SHA-256 记录，不会上传或安装。浏览器检查范围见 [tests/CHECKS.md](tests/CHECKS.md)。
