...
> 仓库 release 与外部目录发布是两条独立状态。确切的 source、package 和公开 listing 状态见[当前状态](docs/current-state.md)。

[官网](https://indeliblevivi.github.io/mcp-boundary/) · [English](README.md) · [统一工作区设计](docs/UNIFIED-WORKSPACE.md) · [0.2.0 提交材料](docs/submission/0.2.0.md) · [许可](LICENSING.md)
...
这条仓库安装命令取得的是所选 Git ref 上的源码版本，与外部目录中已发布的版本不同。若要在候选版进入 `main` 前检查它，将 `main` 替换成对应分支名。安装后重启 Codex。
