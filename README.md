...
> Repository releases and external directory publication are separate. See [current state](docs/current-state.md) for the exact source, package, and public listing status.

[Website](https://indeliblevivi.github.io/mcp-boundary/) · [中文说明](README.zh-CN.md) · [Workspace design](docs/UNIFIED-WORKSPACE.md) · [Submission packet](docs/submission/0.2.0.md) · [Licensing](LICENSING.md)
...
This repository command installs the source at the selected Git ref; it is distinct from the externally published directory version. To inspect a candidate before it reaches `main`, replace `main` with that branch name. Restart Codex after installation.
