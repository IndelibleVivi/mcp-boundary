...
[Website source](site/) · [中文说明](README.zh-CN.md) · [Current state](docs/current-state.md) · [Licensing](LICENSING.md)
...
Restart Codex after installation so the new plugin is discovered. The committed portable package lives at [`plugins/mcp-boundary/`](plugins/mcp-boundary/); its root manifest is `plugin.json`.
...
src/plugin/plugin.json     author source for the portable plugin manifest
...
`src/` is the authoring truth. `plugins/mcp-boundary/` is generated by `scripts/build_plugin.py` and must match it byte-for-byte for the files it distributes.
...
The final command creates a deterministic local ZIP and SHA-256 record under ignored `dist/`. It does not upload or install anything.
...
The website uses no analytics, remote fonts, or external runtime assets. Its external links activate only when a visitor follows them.
