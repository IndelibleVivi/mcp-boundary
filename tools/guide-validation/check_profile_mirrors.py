#!/usr/bin/env python3
"""Verify byte-identical historical skill mirrors of registered profiles."""

# Modified for MCP Boundary: the historical write mode was removed so current
# validation cannot mutate the frozen mcp-server-engineering skill snapshot.
...
def main() -> int:
    ...
    for group_name, index, entry in entries:
        ...
        for locale in LOCALES:
            ...
            try:
                canonical = safe_file(root, paths.get(locale), f"{label}.paths.{locale}")
                mirror = safe_file(root, skill_paths.get(locale), f"{label}.skill_paths.{locale}")
    ...
    for report in reports:
        print(report)
    print(f"PASS: verified {len(reports)} historical profile mirrors")
    return 0
